...
class SettingsWidget(QWidget):
    ...
    def update_setting(self, key, value, parent_key=None):
        ...
        self.mwindow.chatbox.gpt_connector.settings = self.gpt_settings_instance
        ...
        for key, value in data.items():
            ...
            if isinstance(value, bool):
                ...
                widget.stateChanged.connect(
                    lambda state, k=key: self.update_setting(k, bool(state), parent_key)
                )
                ...
                widget.textChanged.connect(
                    lambda text, k=key: self.update_setting(k, text, parent_key)
                )
                ...
                if use_spin_box:
                    widget.valueChanged.connect(
                        lambda val, l=label, k=key: (
                            self.update_setting(k, round(val, step_digits), parent_key),
                            l.setText(k + ": " + str(round(val, step_digits))),
                        )
                    )
                else:
                    widget.valueChanged.connect(
                ...
                if use_spin_box:
                    ...
                    widget.valueChanged.connect(
                        lambda val, l=label, k=key: (
                            self.update_setting(
                                k, round(val / 10, step_digits), parent_key
                            ),
                            l.setText(k + ": " + str(round(val / 10, step_digits))),
                        )
                    )
